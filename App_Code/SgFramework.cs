...
namespace SgFramework
{
    public class SgUtil
    {
        ConnectionStringSettings cts = ConfigurationManager.ConnectionStrings["DBConnectionString"];
        private String connStr = "";
        public SqlConnection TranConn;
        public SqlCommand TranCmd;
        ...
        public SgUtil(ConnectionStringSettings admincts)
        {
            connStr = cts.ConnectionString.ToString();
        }

        #region Base64Encoding/Decoding
        ...
        #region ZIP압축 AddZip (압축폴더,ZIP파일경로및파일명,비밀번호)
        public bool AddZip(string targetFolderPath, string zipFilePath, string password)
        {
            bool retVal = false;
                ...
                try
                {
                    ...
                    foreach (string Fil in ar)
                    {
                        ...
                        if (!Fil.EndsWith(@"/"))
                        {
                            ...
                            ostream.Dispose(); // 이미지 Object 해제처리
                            ostream = null;
                        }
                    }
        ...
        #region ZIP압축 AddZip (임시디렉토리,압축폴더,ZIP파일경로및파일명,비밀번호)
        public bool AddZip(String TempDir, List<string> targetFiles, string zipFilePath)
        {
            return AddZip(TempDir, targetFiles, zipFilePath, "");
        }

        public bool AddZip(String TempDir, List<string> targetFiles, string zipFilePath, string password)
        {
            bool retVal = true;
            String targetFolderPath = TempDir;

            // 압축 대상 폴더의 파일 목록.
            String TempFileName = "ziptemp_" + DateTime.Now.ToString("yyyyMMddHHmmss");


            for (int i = 0; i < targetFiles.Count; i++)
            {
                if (File.Exists(targetFiles[i].ToString()) == true)
                {
                    File.Copy(targetFiles[i].ToString(), targetFolderPath + TempFileName + "_" + i.ToString() + "." + FileExt(targetFiles[i].ToString()), true);
                }
            }
            ...
            try
            {
                ...
                for (int i = 0; i < targetFiles.Count; i++)
                {
                    oZipEntry = new ZipEntry(targetFiles[i].Remove(0, TrimLength));
                    oZipStream.PutNextEntry(oZipEntry);

                    // 파일인 경우.
                    if (!targetFiles[i].EndsWith(@"/"))
                    {
                        ostream = File.OpenRead(targetFolderPath + TempFileName + "_" + i.ToString() + "." + FileExt(targetFiles[i].ToString()));
                        obuffer = new byte[ostream.Length];
                        ostream.Read(obuffer, 0, obuffer.Length);
                        oZipStream.Write(obuffer, 0, obuffer.Length);
                        ostream.Dispose(); // 이미지 Object 해제처리
            ...
            finally
            {
                ...
                for (int i = 0; i < targetFiles.Count; i++)
                {
                    FileDelete(targetFolderPath + TempFileName + "_" + i.ToString() + "." + FileExt(targetFiles[i].ToString()));
                }
            }
        ...
        #region ZIP압축해제 UnZip ( zipFilePath,unZipTargetFolderPath,password)
        public bool UnZip(string zipFilePath, string unZipTargetFolderPath, string password)
        {
            bool retVal = false;
            // ZIP 파일이 있는 경우만 수행.
        ...
        #region ZIP압축해제 UnZip (Stream str, String unZipTargetFolderPath)
        public bool UnZip(Stream str, String unZipTargetFolderPath)
        {
            bool retVal = false;

            // ZIP 스트림 생성.
            ZipInputStream zipInputStream = new ZipInputStream(str);
        ...
        #region Debug 함수 처리 isDebug(page,bool)
        public void isDebug(Page sp, bool pm)
        {
            this.p = sp;
            this.DebugView = pm;
        }

        public void rw(String strVal)
        {
            if (this.DebugView)
            {
                p.Response.Write(strVal);
            }
        }
        #endregion
        ...
        public int binding(DropDownList obj, Dictionary<string, string> dic)
        {
            int retVal = 0;
            String setVal = "";
            ...
            try
            {
                ...
                {
                    obj.Items.Add(new ListItem(kvp.Value, kvp.Key));
                }

                if (setVal != "" || dic.Count == 1)
                {
                    setDropDownList(obj, setVal);
                }
        ...
        #region alert : javascript alert 함수처리
        public void alert(Page currentPage, string message)
        {
            if (!currentPage.ClientScript.IsStartupScriptRegistered(currentPage.GetType(), "Alert"))
            {
                string strScript = @"alert('" + message + "');";
                currentPage.ClientScript.RegisterStartupScript(currentPage.GetType(), "Alert", strScript, true);
            }
        }
        #endregion
        ...
        #region SqlDsQuery : 결과 DataSet
        public DataSet SqlDsQuery(string sql)
        {
            DataSet ds = null;
        ...
        #region SqlDrQuery : 결과 SqlDataReader
        public SqlDataReader SqlDrQuery(string sql)
        {
        ...
        #region SqlScalarQuery : 결과 int
        public int SqlScalarQuery(string sql)
        {
            int retVal = 0;
        ...
        #region 날짜에서 토/일 건너뛴 날짜를 반환
        public DateTime 다음영업일리턴(DateTime 체크일자)
        {
            while (true)
            {
                String 요일 = 체크일자.Date.ToString("ddd");
                if (요일 == "토" || 요일 == "일")
                {
                    체크일자 = 체크일자.AddDays(1);
                }
        ...
        {
            String UserID = ReqS(P, "userid");

            String AdminCnt = SqlFieldQuery("select count(*) from dbo.member where userid='" + UserID + "' and lev='s' ");
            String cnt = SqlFieldQuery("select count(*) from dbo.t_메뉴 where id='" + PageNo + "' and 공통메뉴여부='Y' ");
            ...
            if (cnt == "0")
            {
                ...
                if (AdminCnt == "0")
                {
                    String sql = "select count(*) from dbo.t_메뉴권한 where 직원ID='" + UserID + "' and 메뉴ID='" + PageNo + "' ";
        ...
        public bool PermitCheck(Page P, String PageNo)
        {
            ...
            String cnt = SqlFieldQuery("select count(*) from dbo.t_메뉴 with (NOLOCK) where id='" + PageNo + "' and 공통메뉴여부='Y' ");
            // 공통 메뉴일 경우 권한체크 안함
            if (cnt == "0")
            {

                String PageChk = SqlFieldQuery("select count(*) from dbo.t_메뉴권한 with (NOLOCK) where 직원ID='" + UserID + "' and 메뉴ID='" + PageNo + "' ");
                if (PageChk == "0")
                {
                    retVal = false;
                }
                ...
                //String AdminCnt = SqlFieldQuery("select count(*) from dbo.직원2 where ID='" + UserID + "' and 구분='4' ");
                //if (AdminCnt != "0")
                //{
                //    retVal = true;
                //}
        ...
        // 전산 관리자 체크 여부
        public bool PermitAdmin(Page P)
        {
            bool retVal = false;
            String UserID = AuthUser.gUserID;

            int AdminCnt = SqlScalarQuery("select count(*) from dbo.직원2 with (NOLOCK) where ID='" + UserID + "' and 구분='4' ");
            if (AdminCnt != 0)
            {
                retVal = true;
            }
        ...
        {
            String sender = "";
            DataView dv = SqlDvQuery("select 이메일, 이름 from dbo.t_자동메일권한 with(nolock) where 삭제일시 is null and 메일id='" + MailNo + "' order by 이름");
        ...
        #region ImageCodecInfo GetCodecInfo
        ...
        public String SetSMS(String NumFrom, String DESC, String UserID)
        {
            ...
            SqlNoneQuery2("INSERT INTO [dbo].[t_문자발송_발송번호](num_from,num_desc,user_id) values('" + NumFrom + "','" + DESC + "','" + UserID + "')");
        ...
        public String SendPUSH(String token, String title, String msg, String link, String UserID)
        {
            String retVal = "";

            String url = "https://api.smart36.co.kr/SMS/Send_Push.aspx";
            Dictionary<String, String> pdata = new Dictionary<string, string>();
            pdata.Add("token", token);
            pdata.Add("title", title);
            pdata.Add("msg", msg);
            pdata.Add("link", link);
            pdata.Add("userid", UserID);

            retVal = sghttp_post(url, pdata);
        ...
        #region 포인트 관련 함수
        public String 포인트_잔액(String 계약번호)
        {
            String 잔여포인트 = "0";

            잔여포인트 = SqlFieldQuery("select isnull(sum(금액),0) from dbo.tbl_포인트 where 계약번호='" + 계약번호 + "' and 사용일시<=convert(varchar(10),getdate(),121) ");

            return 잔여포인트;
        }

        public String 포인트_차감(String 계약번호, String 차감금액, String 등록사유, String 등록자ID, String 등록IP, String 사용업체ID)
        {
            String retVal = "";

            try
            {
                String 잔여포인트 = 포인트_잔액(계약번호);

                if (Convert.ToInt64(잔여포인트) >= Convert.ToInt64(차감금액))
                {
                    잔여포인트 = (Convert.ToInt64(잔여포인트) - Convert.ToInt64(차감금액)).ToString();
                    String 검증키 = Encrypt_SHA(계약번호 + 차감금액 + 잔여포인트 + DateTime.Now.ToString("yyyyMMdd"));

                    String sql = "insert into dbo.tbl_포인트(계약번호,금액,잔액,등록자ID,등록사유,등록IP,검증키,사용업체id,사용일시) ";
                    sql += "values('" + 계약번호 + "','-" + 차감금액 + "','" + 잔여포인트 + "','" + 등록자ID + "','" + 등록사유 + "','" + 등록IP + "','" + 검증키 + "' ,'" + 사용업체ID + "',convert(varchar(10),getdate(),121)); select @@identity; ";
                    retVal = SqlFieldQuery(sql);
                }
            }
            catch (Exception ex)
            {
        ...
        public String 포인트_차감(String 계약번호, String 차감금액, String 등록사유, String 등록자ID, String 등록IP, String 사용업체ID, String 입금정보ID)
        {
            String retVal = "";
            ...
            try
            {
                ...
                if (Convert.ToInt64(잔여포인트) >= Convert.ToInt64(차감금액))
                {
                    ...
                    String sql = "insert into dbo.tbl_포인트(계약번호,금액,잔액,등록자ID,등록사유,등록IP,검증키,사용업체id,사용일시, 입금정보ID) ";
                    sql += "values('" + 계약번호 + "','-" + 차감금액 + "','" + 잔여포인트 + "','" + 등록자ID + "','" + 등록사유 + "','" + 등록IP + "','" + 검증키 + "' ,'" + 사용업체ID + "',convert(varchar(10),getdate(),121), '" + 입금정보ID + "'); select @@identity; ";
                    retVal = SqlFieldQuery(sql);
                }
            }
            catch (Exception ex)
        ...
        public String 포인트_충전(String 계약번호, String 충전금액, String 등록사유, String 등록자ID, String 등록IP, String 충전업체ID)
        {
            String retVal = "";

            try
            {
                String 사용일시 = DateTime.Now.ToString("yyyy-MM-dd");

                retVal = 포인트_충전(계약번호, 충전금액, 등록사유, 등록자ID, 등록IP, 충전업체ID, 사용일시);
        ...
        public String 포인트_충전(String 계약번호, String 충전금액, String 등록사유, String 등록자ID, String 등록IP, String 충전업체ID, String 사용일시)
        {
            String retVal = "";
            ...
            try
            {
                ...
                잔여포인트 = (Convert.ToInt64(잔여포인트) + Convert.ToInt64(충전금액)).ToString();
                String 검증키 = Encrypt_SHA(계약번호 + 충전금액 + 잔여포인트 + DateTime.Now.ToString("yyyyMMdd"));

                String sql = "insert into dbo.tbl_포인트(계약번호,금액,잔액,등록자ID,등록사유,등록IP,검증키,충전업체id,사용일시) ";
                sql += "values('" + 계약번호 + "','" + 충전금액 + "','" + 잔여포인트 + "','" + 등록자ID + "','" + 등록사유 + "','" + 등록IP + "','" + 검증키 + "' ,'" + 충전업체ID + "', '" + 사용일시 + "'); select @@identity; ";
                retVal = SqlFieldQuery(sql);
        ...
        public String 포인트_충전(String 계약번호, String 충전금액, String 등록사유, String 등록자ID, String 등록IP, String 충전업체ID, String 사용일시, String 입금정보ID)
        {
            String retVal = "";
            ...
            try
            {
                ...
                String sql = "insert into dbo.tbl_포인트(계약번호,금액,잔액,등록자ID,등록사유,등록IP,검증키,충전업체id,사용일시,입금정보ID) ";
                sql += "values('" + 계약번호 + "','" + 충전금액 + "','" + 잔여포인트 + "','" + 등록자ID + "','" + 등록사유 + "','" + 등록IP + "','" + 검증키 + "' ,'" + 충전업체ID + "', '" + 사용일시 + "', '" + 입금정보ID + "'); select @@identity; ";
                retVal = SqlFieldQuery(sql);
        ...
        #region 배송관련처리 함수
        public String f_발주처리(String 주문번호, String 상품순번, String 발주요청일)
        {
            String retVal = "";

            try
            {
                // 기존 상품 순번에 대해 발주데이터가 요청/대기/진행/완료 가 있는지 체크
                String chk = "select counr(*) from dbo.tbl_배송요청 with(nolock) where 주문번호='" + 주문번호 + "' and 상품순번='" + 상품순번 + "' and 배송상태 in ('배송대기','배송진행','배송완료')";
                if (chk == "0")
                {
                ...
                {
                    retVal = "이미 배송대기,배송진행,배송완료건이 있으므로 재발주가 불가능합니다. 기존 내용을 처리후에 재발주 가능합니다.";
                }
        ...
        #region 상태값 별 건수 체크(계약완료 건 중 체크)

        public String CallStatus_Cnt(String cName, String cHp, String Status)
        {
            String cnt = "";
            String sql = "";

            sql = " select COUNT(*) from v_계약정보 with (NOLOCK) where 1=1 ";
            sql += " and ( 계약자연락처='" + cHp + "' or 계약자휴대폰='" + cHp + "') ";
            sql += " and 계약자명='" + cName + "' ";
            sql += " and 콜상태='" + Status + "' ";

            cnt = SqlFieldQuery(sql);
        ...
        #region RESTAPI RAW BODY 가져오기
        public String getRawBody(Page p)
        {
            string documentContents;
            using (Stream receiveStream = p.Request.InputStream)
            {
                using (StreamReader readStream = new StreamReader(receiveStream, p.Request.ContentEncoding))
                {
